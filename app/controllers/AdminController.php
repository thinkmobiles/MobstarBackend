...
class AdminController extends BaseController
{
	...
	{
		$entries = $this->entry->all_include_deleted(0,0,0,0,'entry_id', 'desc', 10000);
		$data[ 'entries' ] = [ ];
		...
		foreach( $entries as $entry )
		{
			...
			foreach( $entry->file as $file )
			{
				if( $entry->entry_type == 'video' )
				{
					...
					{
						$new[ 'entry_file' ] = $client->getObjectUrl( Config::get('app.bucket'), $file->entry_file_name . "." . $file->entry_file_type, '+60 minutes' );
						$new[ 'entry_image' ] = $client->getObjectUrl( Config::get('app.bucket'), 'thumbs/' . $file->entry_file_name . '-thumb.jpg', '+60 minutes' );
				...
				elseif( $entry->entry_type == 'audio' )
				{
					...
					else
					{
						...
						{
							$new[ 'entry_image' ] = $client->getObjectUrl( Config::get('app.bucket'), $file->entry_file_name . "." . $file->entry_file_type, '+60 minutes' );
						}
					}
				}
