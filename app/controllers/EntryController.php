...
//			$current[ 'user' ][ 'isMyStar' ] = Star::where( 'user_star_user_id', '=', $session->user_id )->where( 'user_star_star_id', '=', $entry->entry_user_id )->count();
	...
	public function mysearch()
	{
		...
		->select('entries.*')
		->join('users', 'entries.entry_user_id', '=', 'users.user_id')
        ->where('entries.entry_name', 'LIKE', '%Roy Moore%')
        ->orWhere(function($query)
            {
                $query->orWhere('entries.entry_description', 'LIKE', '%Roy Moore%')
						->orWhere('users.user_name', 'LIKE', '%Roy Moore%')
						->orWhere('users.user_full_name', 'LIKE', '%Roy Moore%');
            })
		->where('entries.entry_deleted', '=', '0')
        ->get();	
		dd(DB::getQueryLog());
